  ...
  describe 'config' do
    context 'On Debian with valid params' do
      ...
      it_behaves_like 'a Linux OS' do
        ...
        it { should contain_file('/etc/lam/config.php') }
        it { should contain_file('/etc/lam/config.php').with_ensure('file') }
        it { should contain_file('/etc/lam/config.php').with_mode('0640') }
        it { should contain_file('/etc/lam/config.php').with_owner('root') }
        it { should contain_file('/etc/lam/config.php').with_group('www-data') }
        it { should contain_file('/etc/lam/config.php')
    ...
    context 'On CentOS with valid params' do
      ...
      it_behaves_like 'a Linux OS' do
        ...
        it { should contain_file('/etc/lam/config.php').with_mode('0640') }
        it { should contain_file('/etc/lam/config.php').with_owner('root') }
        it { should contain_file('/etc/lam/config.php').with_group('apache') }
        it { should contain_file('/etc/lam/config.php')
          .with_content(/\$servers->setValue\('server','host', 'localhost'\);/)
        }
        it { should contain_file('/etc/lam/config.php')
